        ...
        <div className={`${escss.containerE} container-1400 py-5`} >
            <div className={`d-lg-flex align-items-strech ${escss.beforeRoundE}`}>
                ...
                <ul className={`${escss.eventListE} d-md-flex justify-content-between px-0 position-relative`}>
                    ...
                    {
                        esdata.map(( list, listnumber )=>{
                            return(
                                <li className={`overflow-hidden  bg-white mx-4 px-0 ${
                                    listnumber === 0 ? escss.eventFirstE : 
                                    listnumber === 1 ? escss.eventSecondE : 
                                    ""
                                  } `} >
                                <a href={list.linkHref} className={`d-block overflow-hidden ${escss.thumbImgE}`}><img src={list.imageSrc} alt={list.alt} className="d-block img-fluid" /></a>
                                <a href={list.linkHref} className=" d-block ">
                                    <strong className={`${escss.textTitleE} d-block mb-3`}>{list.title}</strong>
                                    <span className="d-block">{list.description}
                                    </span>
